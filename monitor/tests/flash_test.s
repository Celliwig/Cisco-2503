###########################################################################
#                                                                         #
#                Test access to Flash command registers                   #
#                                                                         #
###########################################################################
...
.org	0x40
flash_main_test:
	movea.l	#0x03000000, %a0					/* Main Flash ROM base address */
	move.w	0x2329, %d2						/* Delay loop value */
	move.l	#0x15554, %d1
	move.l	#0x55555555, (%a0,%d1.l)
	move.l	#0xaaa8, %d0
	move.l	#0xaaaaaaaa, (%a0,%d0.l)
	move.l	#0x9090909, (%a0,%d1.l)
	tst.w	%d2
	beq.s	flash_main_test_get_rslt
flash_main_test_delay_loop:
	subq.w	#1, %d2
	bne.s	flash_main_test_delay_loop
flash_main_test_get_rslt:
	move.l	(%a0), %d6
	addq.w	#4, %a0
	move.l	(%a0), %d7
	move.l	#0xf0f0f0f, (%a0)
	...
	move.w	#0xffff, %d0
flash_test_finish_loop:
	dbra	%d0, flash_test_finish_loop
